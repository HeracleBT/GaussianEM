...
def show_gaussians(
    ...
    density_scale = 0.75 / g_density.max()
    ...
    for i_ellipse in trange(n_gaussian, desc="visualize gaussians"):
        ...
        ellipse = create_o3d_ellipse(
            ...
            g_scale[i_ellipse],
            g_quad[i_ellipse],
            (1 - g_density[i_ellipse] * density_scale),
            resolution=mesh_resoltuion,
        )
        # ellipse.paint_uniform_color([1, 0.706, 0])
...
def create_o3d_ellipse(center, scale, quad, density, resolution=5):
    ...
    vertex_colors = np.ones_like(np.asarray(mesh.vertices)) * 0.75
    mesh.vertex_colors = o3d.utility.Vector3dVector(vertex_colors)

    mesh = mesh.transform(transform)

    # mat = o3d.visualization.rendering.MaterialRecord()
    # mat.shader = "defaultLitTransparency"  # 使用非光照着色器可以更好地显示透明效果
    # mat.base_color = [density, density, density, 0.4]
    # geom = {'name': density, 'geometry': mesh, 'material': mat}
