...
def parse_star(relionfile: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Read the data table in a .star file, and the data optics table if present."""
    if not relionfile.endswith(".star"):
        raise ValueError(f"{relionfile} is not a .star file!")

    comments = list()
    blocks = dict()
    cur_block = None
    with open(relionfile, "r") as f:
        while True:
            line = f.readline()
            if not line:
                break
    ...
    if data_block is None:
        raise ValueError(f"relionfile `{relionfile}` does not contain a data block!")

    return data_block, blocks["data_optics"] if "data_optics" in blocks else None


def write_star(
    relionfile: str, data: pd.DataFrame, data_optics: Optional[pd.DataFrame] = None
) -> None:
    """Save star data to file, using RELION3.1 or RELION3.0 format if w/o optics."""
    with open(relionfile, "w") as f:
        f.write("# Created {}\n".format(dt.now()))
        f.write("\n")
...
class Relionfile:
    ...
    > relionfile = relionfile("mydata_folder/.particles.star")

    # If using data tables, must use keywords
    > relionfile = relionfile(data=stardf, data_optics=optics_df)

    # Can also override data optics table found in file (but not `data` as well!)
    > relionfile = relionfile("mydata_folder/.particles.star", data_optics=optics_df)

    """

    def __init__(
        self,
        relionfile: Optional[str] = None,
        *,
        data: Optional[pd.DataFrame] = None,
        data_optics: Optional[pd.DataFrame] = None,
    ):
        if (relionfile is None) == (data is None):
            raise ValueError(
                f"relionfile must be instantiated with "
                f"exactly one of {relionfile} or {data}!"
            )
        if relionfile is not None:
            data, data_optics = parse_star(relionfile)
    ...
    @classmethod
    def load(cls, relionfile: str) -> Self:
        """Convenience instantiation method for backwards compatibility."""
        return cls(relionfile=relionfile)
...
class StarfileSource(_MRCDataFrameSource, Relionfile):
    ...
    def __init__(
        ...
        Relionfile.__init__(self, data=sdata, data_optics=data_optics)
        self.df = None
